document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadSchoolComputersData(dataRows, headers) {
        ...
        dataRows.forEach((row, index) => {
            // Check if the row is valid (has enough columns based on headers)
            if (row.length < headers.length) {
                console.warn(`Skipping invalid row at index ${index}:`, row);
                return; // Skip this row if it's malformed
            }

            const schoolTypeIndex = headers.indexOf('구분');
            ...
            if ([schoolTypeIndex, totalIndex, studentIndex, teacherIndex, staffIndex, otherIndex].includes(-1)) {
                console.error('One or more headers not found in:', headers);
                return; // Stop processing if headers are missing
            }

            const schoolType = row[schoolTypeIndex];
            ...
            // Log data specifically for '서울특별시'
            if (schoolType === '서울특별시') {
                console.log('Processing 서울특별시 row:', row);
                console.log('  School Type:', schoolType, '(Index:', schoolTypeIndex, ')');
                console.log('  Total:', total, '(Index:', totalIndex, ')');
                console.log('  Student:', student, '(Index:', studentIndex, ')');
                console.log('  Teacher:', teacher, '(Index:', teacherIndex, ')');
                console.log('  Staff:', staff, '(Index:', staffIndex, ')');
                console.log('  Other:', other, '(Index:', otherIndex, ')');
            }
